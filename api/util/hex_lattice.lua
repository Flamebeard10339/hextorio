...
---Whole quarter turns are applied exactly, so that positions on the lattice stay on the tile grid.
---@param x number
---@param y number
...
local function rotate(x, y, rotation)
    ...
    local turns = round(quarter_turns)
...
local function derive_lattice(scale)
    local ideal_half_width = math.max(1, scale * ROOT_THREE_OVER_TWO)
    local best ---@type HexLattice
    local best_cost

    for half_width = math.max(1, round(ideal_half_width) - 1), round(ideal_half_width) + 1 do
        local regular_height = ROOT_THREE * half_width
        local height = math.max(1, round(regular_height))
...
function hex_lattice.get_hex_center(hex_pos, axial_scale, axial_rotation)
    ...
    x, y = rotate(x, y, axial_rotation or 0)
...
function hex_lattice.get_fractional_hex(rect_pos, axial_scale, axial_rotation)
    ...
    local x, y = rotate(rect_pos.x, rect_pos.y, -(axial_rotation or 0))

    local r = y / lattice.height
    local q = x / lattice.width - r * 0.5
...
function hex_lattice.get_hex_corners(hex_pos, axial_scale, axial_rotation, hex_size_decrement)
    ...
    for i = 1, 6 do
        ...
        local x, y = rotate(center.x + offset[1] * half_width, center.y + offset[2] * third_height, axial_rotation)
